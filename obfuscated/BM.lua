...
do
	...
	do
		local function ZukaTech()
			...
			local COMBINING_COUNT = #COMBINING
			local DIRECTIONAL = {
				"\xE2\x80\x8F",
				"\xD2\x89",
				"\xCC\xA9",
			...
			local function zalgoChar(ch, id, pos)
				local result = ch
				local count = (hash(id * 17 + pos, pos) % 3) + 1
				for i = 1, count do
					local rawIdx = combOrder[(hash(id * 31 + pos * 7 + i, i) % #combOrder) + 1]
					local ci = (rawIdx - 1) % COMBINING_COUNT + 1
					result = result .. COMBINING[ci]
				end
				if hash(id + pos * 13, pos + 1) % 4 == 0 then
			...
			local function prepare(ast)
				utils.shuffle(VarDigits)
				utils.shuffle(VarStartDigits)
				seed = math.random(0, 0xFFFFFFFF)
				combOrder = {}
				for i = 1, COMBINING_COUNT do
					combOrder[i] = i
				end
				utils.shuffle(combOrder)
			end
			return { generateName = generateName, prepare = prepare }
		end
	...
	do
		local function ZukaTech()
			return {
				...
				MoonSec  = (function()
					-- Single lowercase letter names, cycling a-z then aa-zz etc.
					-- Matches MoonSec V3 visual style exactly.
					...
					local function buildPool()
						pool = {}
						local used = {}
						-- First: all 26 single letters (shuffled)
						local singles = {}
						for _, c in ipairs(LETTERS) do singles[#singles+1] = c end
						...
						-- Then: two-letter combos (shuffled)
						local doubles = {}
						...
						for _, s in ipairs(doubles) do pool[#pool+1] = s end
					end
					local function generateName()
						if idx > #pool then
							-- overflow: append random digit
							local base = LETTERS[math.random(1,26)]
							return base .. tostring(math.random(0,9))
						end
						local name = pool[idx]; idx = idx + 1
						return name
					end
					local function prepare()
						idx = 1
				...
				local aiResistGenerators = {
					function(val, depth)
						...
						local lenCall = Ast.FunctionCallExpression(
							Ast.IndexExpression(Ast.StringExpression("string"), Ast.StringExpression("len")),
					...
					function(val, depth)
						...
						local floorCall = Ast.FunctionCallExpression(
							Ast.IndexExpression(Ast.StringExpression("math"), Ast.StringExpression("floor")),
					...
					function(val, depth)
						...
						local maxCall = Ast.FunctionCallExpression(
							Ast.IndexExpression(Ast.StringExpression("math"), Ast.StringExpression("max")),
					...
					function(val, depth)
						...
						local absCall = Ast.FunctionCallExpression(
							Ast.IndexExpression(Ast.StringExpression("math"), Ast.StringExpression("abs")),
		...
		local function ZukaTech()
			...
			function BC:compileStmt(proto, stmt, funcDepth, getVarReg, varargReg, envR)
				...
				if k == AstKind.ForInStatement then
					...
					local stateR = exprs[2] and self:compileExpr(
						proto,
						exprs[2],
						funcDepth,
						getVarReg,
					...
					local ctrlR = exprs[3] and self:compileExpr(
						proto,
						exprs[3],
						funcDepth,
						getVarReg,
			...
			function VmifyBC:apply(ast, pipeline)
				...
				local glue =
					table.concat({
						...
						"    return table.concat(t)",
						"end",
						"local _blob=_xdB(_xdB(_lzwD(" .. blobLit .. ")," .. tostring(rtSalt) .. ")," .. tostring(
							blobKey
						) .. ")",
	...
	do
		local function ZukaTech()
			...
			local function buildFakeVM(xorSeed, fakeOpCount)
				...
				-- scatter re Maining entries as individual assignments
			...
			return {
				["Default"] = {
					...
					Steps = {
						...
						{ Name = "PolyStringEncode", Settings = { Threshold = 0.75, MinLength = 2 } },
						--{ Name = "BootstrapObfuscator", Settings = { StatementsPerState = 2, AddJunkState = true } },
						...
						{
							Name = "DynamicJumps",
							Settings = { Threshold = 0.80, PoolSize = 64, MinStatements = 2 },
						},
						{
						...
						{
							Name = "FakeBlob",
							Settings = { Placement = "after", BlobSize = 912, FakeOpcodeCount = 28 },
						},
				...
				["Standard"] = {
					...
					Seed = 0,
					Steps = {
						{ Name = "HerculesVM", Settings = { ShuffleOpcodes = true, XorKey = 0 } },
						{ Name = "WrapInFunction", Settings = {} },
					},
					Hercules = nil,
				},
				["byte"] = {
					...
					Steps = {
						{
							Name = "VmifyBC",
							Settings = { XorKey = 0, ShuffleOpcodes = true },
						},
						{ Name = "WrapInFunction", Settings = {} },
					},
					Hercules = nil,
...
local function generateStateMachine(seed)
	...
	-- simple flat counter loop, no nested blocks, no locals inside
	out[#out+1] = "do"
...
-- ─────────────────────────────────────────────────────────────────────────────
--  SHIELD RUNTIME WRAP
--  Embeds a stripped-down SHIELD sandbox bootstrap around the obfuscated
--  payload. When the output runs on an executor:
--    1. SHIELD hooks loadstring in the executor's global env
--    2. The obfuscated VM boots and runs normally (checkcaller() == true)
--    3. Any external loadstring call against the output hits the sandbox
-- ─────────────────────────────────────────────────────────────────────────────
local function generateShieldWrap(obfuscatedPayload, seed)
	-- LCG to produce seed-derived variable names — every build looks different to static analysis
	local s = (seed or 0) % 0x100000000
	local function rs()
		s = (s * 1664525 + 1013904223) % 0x100000000
		...
		for i = 1, len do
			s = (s * 1664525 + 1013904223) % 0x100000000
			buf[i] = chars:sub((s % 26) + 1, (s % 26) + 1)
		end
		return "_zt" .. table.concat(buf)
	end

	-- Generate unique identifiers for every structural symbol
	local MI   = rs()  -- makeDecoyInstance
	local MG   = rs()  -- makeDecoyGame
	local RO   = rs()  -- readonly
	local BE   = rs()  -- buildEnv
	local GV   = rs()  -- genv
	local RL   = rs()  -- REAL_LOADSTRING
	local HK   = rs()  -- hook closure
	local FN   = rs()  -- compiled fn inside hook
	local PR   = rs()  -- props table inside makeDecoyInstance
	local MT   = rs()  -- metatable inside makeDecoyInstance
	local SG   = rs()  -- fakeSignal function
	local SM   = rs()  -- svcMap table
	local FM   = rs()  -- fmt (metatable of fakeGame)
	local FO   = rs()  -- forig (original __index of fakeGame)

	-- Build the bootstrap as a table of lines — zero risk of placeholder mismatch
	local L = {}
	local function w(line) L[#L+1] = line end

	w("if _SHIELD then return end")

	-- ── makeDecoyInstance ────────────────────────────────────────────────────
	w("local function "..MI.."(cn,nm)")
	w("  cn=cn or'Instance' nm=nm or cn")
	w("  local "..PR.."={Name=nm,ClassName=cn,Parent=nil,Archivable=false}")
	w("  local function "..SG.."()")
	w("    local n={Disconnect=function()end}")
	w("    return{Connect=function(_,f)return n end,Once=function(_,f)return n end,Wait=function()return nil end}")
	w("  end")
	w("  local "..MT.."={}")
	w("  "..MT..".__index=function(_,k)")
	w("    if "..PR.."[k]~=nil then return "..PR.."[k] end")
	w("    if k=='GetChildren'or k=='GetDescendants'then return function()return{}end end")
	w("    if k=='FindFirstChild'or k=='FindFirstChildOfClass'or k=='FindFirstChildWhichIsA'")
	w("    or k=='FindFirstAncestor'or k=='FindFirstAncestorOfClass'then return function()return nil end end")
	w("    if k=='IsA'then return function(_,c)return c==cn or c=='Instance'end end")
	w("    if k=='IsDescendantOf'or k=='IsAncestorOf'then return function()return false end end")
	w("    if k=='Clone'then return function()return "..MI.."(cn,nm..'_clone')end end")
	w("    if k=='Destroy'or k=='Remove'or k=='ClearAllChildren'then return function()end end")
	w("    if k=='GetFullName'then return function()return'Decoy.'..nm end end")
	w("    if k=='WaitForChild'then return function(_,n,_)return "..MI.."('Instance',n)end end")
	w("    if k=='GetService'then return function(_,sv)return "..MI.."(sv,sv)end end")
	w("    if k=='GetAttribute'or k=='GetAttributes'then return function()return nil end end")
	w("    if k=='HasTag'then return function()return false end end")
	w("    if k=='GetTags'then return function()return{}end end")
	w("    if k=='Changed'or k=='ChildAdded'or k=='ChildRemoved'or k=='DescendantAdded'")
	w("    or k=='DescendantRemoving'or k=='AncestryChanged'or k=='AttributeChanged'")
	w("    or k=='Destroying'or k=='Died'or k=='HealthChanged'then return "..SG.."()end")
	w("    if k=='Health'or k=='MaxHealth'then return 100 end")
	w("    if k=='WalkSpeed'then return 16 end")
	w("    return "..MI.."('Instance',tostring(k))")
	w("  end")
	w("  "..MT..".__newindex=function(_,k,v)"..PR.."[k]=v end")
	w("  "..MT..".__tostring=function()return cn..'('..nm..')[D]'end")
	w("  "..MT..".__metatable='locked'")
	w("  return setmetatable({},"..MT..")")
	w("end")

	-- ── makeDecoyGame ────────────────────────────────────────────────────────
	w("local function "..MG.."()")
	w("  local rlp=game:GetService('Players').LocalPlayer")
	w("  local flp="..MI.."('Player',rlp and rlp.Name or 'LocalPlayer')")
	w("  do")
	w("    local m=getmetatable(flp) local o=m.__index")
	w("    m.__index=function(t,k)")
	w("      if k=='UserId'then return rlp and rlp.UserId or 0 end")
	w("      if k=='Character'then return "..MI.."('Model','Character')end")
	w("      if k=='PlayerGui'then return "..MI.."('PlayerGui','PlayerGui')end")
	w("      if k=='Backpack'then return "..MI.."('Backpack','Backpack')end")
	w("      return o(t,k)")
	w("    end")
	w("  end")
	w("  local fp="..MI.."('Players','Players')")
	w("  do")
	w("    local m=getmetatable(fp) local o=m.__index")
	w("    m.__index=function(t,k)")
	w("      if k=='LocalPlayer'then return flp end")
	w("      if k=='GetPlayers'then return function()return{flp}end end")
	w("      return o(t,k)")
	w("    end")
	w("  end")
	w("  local fr="..MI.."('RunService','RunService')")
	w("  do")
	w("    local m=getmetatable(fr) local o=m.__index")
	w("    local nn={Connect=function(_,f)return{Disconnect=function()end}end}")
	w("    m.__index=function(t,k)")
	w("      if k=='Heartbeat'or k=='RenderStepped'or k=='Stepped'then return nn end")
	w("      if k=='IsClient'then return function()return true end end")
	w("      if k=='IsServer'then return function()return false end end")
	w("      return o(t,k)")
	w("    end")
	w("  end")
	w("  local fh="..MI.."('HttpService','HttpService')")
	w("  do")
	w("    local m=getmetatable(fh) local o=m.__index")
	w("    m.__index=function(t,k)")
	w("      if k=='GetAsync'or k=='PostAsync'or k=='RequestAsync'then")
	w("        return function()error('HttpService blocked by SHIELD')end")
	w("      end")
	w("      return o(t,k)")
	w("    end")
	w("  end")
	w("  local "..SM.."={")
	w("    Workspace="..MI.."('Workspace','Workspace'),")
	w("    Players=fp,")
	w("    ReplicatedStorage="..MI.."('ReplicatedStorage','ReplicatedStorage'),")
	w("    ServerStorage="..MI.."('ServerStorage','ServerStorage'),")
	w("    ServerScriptService="..MI.."('ServerScriptService','ServerScriptService'),")
	w("    StarterGui="..MI.."('StarterGui','StarterGui'),")
	w("    StarterPack="..MI.."('StarterPack','StarterPack'),")
	w("    SoundService="..MI.."('SoundService','SoundService'),")
	w("    HttpService=fh,")
	w("    RunService=fr,")
	w("    TweenService="..MI.."('TweenService','TweenService'),")
	w("    UserInputService="..MI.."('UserInputService','UserInputService'),")
	w("    CoreGui="..MI.."('CoreGui','CoreGui'),")
	w("    Lighting="..MI.."('Lighting','Lighting'),")
	w("    Teams="..MI.."('Teams','Teams'),")
	w("    Chat="..MI.."('Chat','Chat'),")
	w("  }")
	w("  rawset("..SM..".Workspace,'CurrentCamera',"..MI.."('Camera','Camera'))")
	w("  local fg="..MI.."('DataModel','Game')")
	w("  local "..FM.."=getmetatable(fg)")
	w("  local "..FO.."="..FM..".__index")
	w("  "..FM..".__index=function(t,k)")
	w("    if k=='GetService'then")
	w("      return function(_,sv)")
	w("        if "..SM.."[sv]then return "..SM.."[sv]end")
	w("        local d="..MI.."(sv,sv) "..SM.."[sv]=d return d")
	w("      end")
	w("    end")
	w("    if k=='PlaceId'then return 0 end")
	w("    if k=='JobId'then return'00000000-0000-0000-0000-000000000000'end")
	w("    if k=='CreatorId'then return 0 end")
	w("    if "..SM.."[k]then return "..SM.."[k]end")
	w("    return "..FO.."(t,k)")
	w("  end")
	w("  return fg,"..SM..".Workspace")
	w("end")

	-- ── readonly ─────────────────────────────────────────────────────────────
	w("local function "..RO.."(t)")
	w("  return setmetatable({},{__index=t,__newindex=function()end,__metatable='locked'})")
	w("end")

	-- ── buildEnv ─────────────────────────────────────────────────────────────
	w("local function "..BE.."(fg,fw)")
	w("  local env={}")
	w("  env.print=print env.warn=warn env.error=error env.assert=assert")
	w("  env.tostring=function(v)")
	w("    if v==fg then return'DataModel'end")
	w("    if v==fw then return'Workspace'end")
	w("    return tostring(v)")
	w("  end")
	w("  env.tonumber=tonumber env.type=type env.typeof=typeof")
	w("  env.pairs=pairs env.ipairs=ipairs env.next=next env.select=select")
	w("  env.unpack=table.unpack or unpack")
	w("  env.rawget=rawget env.rawset=rawset env.rawequal=rawequal env.rawlen=rawlen")
	w("  env.pcall=pcall env.xpcall=xpcall")
	w("  env.setmetatable=setmetatable env.getmetatable=getmetatable")
	w("  env.newproxy=newproxy env.collectgarbage=function()end")
	w("  env.math="..RO.."(math) env.table="..RO.."(table) env.string="..RO.."(string)")
	w("  env.bit32=bit32 and "..RO.."(bit32) or nil")
	w("  env.utf8=utf8 and "..RO.."(utf8) or nil")
	w("  env.task="..RO.."(task) env.tick=tick env.time=time")
	w("  env.os="..RO.."({clock=os.clock,time=os.time,date=os.date})")
	w("  env.Vector3=Vector3 env.Vector2=Vector2 env.CFrame=CFrame env.Color3=Color3")
	w("  env.UDim2=UDim2 env.UDim=UDim env.Enum=Enum env.Instance=Instance")
	w("  env.TweenInfo=TweenInfo env.NumberSequence=NumberSequence")
	w("  env.ColorSequence=ColorSequence env.Ray=Ray env.RaycastParams=RaycastParams")
	w("  env.game=fg env.workspace=fw env.Workspace=fw")
	w("  env.script="..MI.."('LocalScript','SandboxedScript')")
	w("  env.shared={} env._G={}")
	w("  env.loadstring=function(src,cn)")
	w("    local fn,e=(getgenv().__ZTSHIELD_REAL or loadstring)(src,cn or'sandboxed')")
	w("    if not fn then return nil,e end")
	w("    local nfg,nfw="..MG.."()")
	w("    setfenv(fn,"..BE.."(nfg,nfw))")
	w("    return fn")
	w("  end")
	w("  env.require=function(id)")
	w("    if type(id)=='number'then return{}end")
	w("    return{}")
	w("  end")
	w("  env.load=function()error('load blocked by SHIELD')end")
	w("  local _blocked={'getfenv','setfenv','getgenv','getrenv','getgc',")
	w("    'getinstances','getsenv','getupvalues','getrawmetatable',")
	w("    'setrawmetatable','setreadonly','hookfunction','hookmetamethod',")
	w("    'newcclosure','iscclosure','checkcaller','decompile',")
	w("    'getscriptbytecode','writefile','readfile','loadfile',")
	w("    'syn','fluxus','KRNL_LOADED','_SHIELD'}")
	w("  for _,g in ipairs(_blocked)do env[g]=nil end")
	w("  env._ENV=env")
	w("  return env")
	w("end")

	-- ── loadstring hook + installation ────────────────────────────────────────
	w("local "..GV.."=getgenv()")
	w("local "..RL.."="..GV..".loadstring or loadstring")
	w(""..GV..".__ZTSHIELD_REAL="..RL)
	w("local "..HK.."=newcclosure(function(src,cn)")
	w("  if checkcaller()then return "..RL.."(src,cn)end")
	w("  local "..FN..",_e="..RL.."(src,cn or'untrusted')")
	w("  if not "..FN.." then return nil,_e end")
	w("  local fg,fw="..MG.."()")
	w("  setfenv("..FN..","..BE.."(fg,fw))")
	w("  return "..FN)
	w("end)")
	w(""..GV..".loadstring="..HK)
	w("_SHIELD=true")
...
if arg and arg[0] then
	...
	local presetName = arg[2] and (arg[2]:sub(1, 1) ~= "-") and arg[2] or "Luraph"
	...
	if not inputFile then
		...
		io.stderr:write("Presets: Luraph (default), byte\n")
		os.exit(1)
	end
	...
	local function runSanityCheck(originalSrc, obfuscatedSrc)
		...
		if #obfuscatedSrc < (#originalSrc * 0.10) then
			return false,
	...
	repeat
		...
		if not ok then
			...
			if sane then
				local wrapSeed = math.random(1, 2^31-1)
				local innerPayload = generateStateMachine(wrapSeed) .. result
				finalResult = '([[ZukaTech Obfuscated]]):gsub(".+",function(a) _=a end) ' .. generateShieldWrap(innerPayload, wrapSeed)
				success = true
			else
				lastErr = sanityErr
				io.stderr:write(
