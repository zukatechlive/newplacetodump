...
function ShiftLock:Enable()
	...
	DoNotif("Shift Lock enabled. Press B to toggle.", 3)
end
...
local IsOnMobile = (function()
	...
	if platform == Enum.Platform.None then
		return (ClonedService("UserInputService")).TouchEnabled
			and not (
				(ClonedService("UserInputService")).KeyboardEnabled or (ClonedService("UserInputService")).MouseEnabled
			)
	end
	return false
end)()
...
function _CameraInput()
	...
	function CameraInput.enableCameraToggleInput()
		...
		rmbUpConnection = rmbUp:Connect(function()
			holdPan = false
			if
				tick() - lastRmbDown < MB_TAP_LENGTH
				and (togglePan or (UserInputService:GetMouseDelta()).Magnitude < 2)
			then
...
function _BaseCamera()
	...
	function BaseCamera.new()
		...
		self.defaultSubjectDistance =
			math.clamp(DEFAULT_DISTANCE, player.CameraMinZoomDistance, player.CameraMaxZoomDistance)
		...
		self.playerDevTouchMoveModeChangeConn = (player:GetPropertyChangedSignal("DevTouchMovementMode")):Connect(
			function()
		...
		self.gameSettingsTouchMoveMoveChangeConn = (UserGameSettings:GetPropertyChangedSignal("TouchMovementMode")):Connect(
			function()
	...
	function BaseCamera:UpdateDefaultSubjectDistance()
		if self.portraitMode then
			self.defaultSubjectDistance =
				math.clamp(PORTRAIT_DEFAULT_DISTANCE, player.CameraMinZoomDistance, player.CameraMaxZoomDistance)
		else
			self.defaultSubjectDistance =
				math.clamp(DEFAULT_DISTANCE, player.CameraMinZoomDistance, player.CameraMaxZoomDistance)
		end
	...
	function BaseCamera:OnPointerAction(wheel, pan, pinch, processed)
		...
		if abs(zoomDelta) > 0 then
			...
			elseif FFlagUserFixZoomInZoomOutDiscrepancy then
				if zoomDelta > 0 then
	...
	function BaseCamera:DoPanRotateCamera(rotateAngle)
		local angle = Util.RotateVectorByAngleAndRound(
			self:GetCameraLookVector() * Vector3.new(1, 0, 1),
			rotateAngle,
			math.pi * 0.25
		)
		if angle ~= 0 then
			...
			if self.lastThumbstickRotate then
				if VRService.VREnabled then
					...
					if self.lastVelocity then
						local velocity = (gamepadPan - self.lastThumbstickPos)
							/ (currentTime - self.lastThumbstickRotate)
	...
	local function queryPoint(origin, unitDir, dist, lastPos)
		...
		repeat
			local entryPart, entryPos =
				FindPartOnRayWithIgnoreListSafe(ray(movingOrigin, target - movingOrigin), blacklist, false, true)
			if entryPart then
...
function _Invisicam()
	...
	function Invisicam:Update(dt, desiredCameraCFrame, desiredCameraFocus)
		...
		if USE_STACKING_TRANSPARENCY then
			...
			hitParts = self.camera:GetPartsObscuringTarget({
				headPoint,
				torsoPoint,
			}, ignoreList)
			for i = 1, #hitParts do
			...
			if hitPartCount > 0 then
				perPartTransparencyHeadTorsoHits =
					math.pow(0.5 * TARGET_TRANSPARENCY + 0.5 * TARGET_TRANSPARENCY / hitPartCount, 1 / hitPartCount)
		...
		elseif self.cameraType == Enum.CameraType.Watch then
			if subjectPosition and player and camera then
				...
				if humanoid and humanoid.RootPart then
					...
					if
						self.lastDistanceToSubject
						and self.lastDistanceToSubject == self:GetCameraToSubjectDistance()
					then
...
function _OrbitalCamera()
	...
	function OrbitalCamera:DoGamepadZoom(name, state, input)
		if
			input.UserInputType == self.activeGamepad
			and (input.KeyCode == Enum.KeyCode.ButtonR3 or input.KeyCode == Enum.KeyCode.ButtonL3)
		then
			if state == Enum.UserInputState.Begin then
	...
	function OrbitalCamera:Update(dt)
		...
		if subjectPosition and player and camera then
			...
			if VREnabled and (not self:IsInFirstPerson()) then
				...
				if self.useAzimuthLimits then
					self.curAzimuthRad =
						math.clamp(self.curAzimuthRad, self.minAzimuthAbsoluteRad, self.maxAzimuthAbsoluteRad)
				else
				...
				self.curElevationRad =
					math.clamp(self.curElevationRad + self.rotateInput.y, self.minElevationRad, self.maxElevationRad)
	...
	function ClassicCamera:Update()
		...
		if subjectPosition and player and camera then
			...
			if self:GetIsMouseLocked() and (not self:IsInFirstPerson()) then
				...
				local cameraRelativeOffset = offset.X * newLookCFrame.rightVector
			...
			elseif not self.userPanningTheCamera and self.lastCameraTransform then
				...
				elseif
					self.isFollowCamera
					and (not (isInFirstPerson or userRecentlyPannedCamera) and not VRService.VREnabled)
				then
			...
			if not self.isFollowCamera then
				...
				newCameraCFrame = CFrame.new(newCameraFocus.p - zoom * newLookVector, newCameraFocus.p)
...
function _CameraUtils()
	...
	function CameraUtils.ConvertCameraModeEnumToStandard(enumValue)
		...
		if
			...
			or enumValue == Enum.DevTouchCameraMovementMode.Classic
			or enumValue == Enum.DevComputerCameraMovementMode.Classic
			or enumValue == Enum.ComputerCameraMovementMode.Classic
		then
			return Enum.ComputerCameraMovementMode.Classic
		end
		if
			enumValue == Enum.TouchCameraMovementMode.Follow
			or enumValue == Enum.DevTouchCameraMovementMode.Follow
			or enumValue == Enum.DevComputerCameraMovementMode.Follow
			or enumValue == Enum.ComputerCameraMovementMode.Follow
		then
			return Enum.ComputerCameraMovementMode.Follow
		end
		if
			enumValue == Enum.TouchCameraMovementMode.Orbital
			or enumValue == Enum.DevTouchCameraMovementMode.Orbital
			or enumValue == Enum.DevComputerCameraMovementMode.Orbital
			or enumValue == Enum.ComputerCameraMovementMode.Orbital
		then
		...
		if FFlagUserCameraToggle then
			if
				...
				or enumValue == Enum.DevComputerCameraMovementMode.CameraToggle
			then
		...
		if
			...
			or enumValue == Enum.DevComputerCameraMovementMode.UserChoice
		then
	...
	function CameraModule:ActivateCameraController(cameraMovementMode, legacyCameraType)
		...
		if not newCameraCreator then
			if
				...
				or FFlagUserCameraToggle and cameraMovementMode == Enum.ComputerCameraMovementMode.CameraToggle
			then
	...
	function CameraModule:OnLocalPlayerCameraPropertyChanged(propertyName)
		if propertyName == "CameraMode" then
			if Players.LocalPlayer.CameraMode == Enum.CameraMode.LockFirstPerson then
				if
					not self.activeCameraController
					or self.activeCameraController:GetModuleName() ~= "ClassicCamera"
				then
	...
	function Camera:Update(dt)
		if self.activeCameraController then
			...
			newCameraFocus = newCameraFocus
				- newCameraCFrame:VectorToWorldSpace(lockOffset)
...
function _ClickToMoveDisplay()
	...
	local function CreateWaypointTemplates()
		...
		FailureWaypointImageLabel.Position =
			UDim2.new(0, -WAYPOINT_BILLBOARD_SIZE.X.Offset / 2, 0, -WAYPOINT_BILLBOARD_SIZE.Y.Offset)
...
function _TouchJump()
	...
	function TouchJump:Create()
		...
		self.jumpButton.Position = isSmallScreen and UDim2.new(1, -(jumpButtonSize * 1.5 - 10), 1, -jumpButtonSize - 20)
...
function _ClickToMoveController()
	...
	local FFlagUserNavigationClickToMoveSkipPassedWaypointsSuccess, FFlagUserNavigationClickToMoveSkipPassedWaypointsResult = pcall(
		function()
			return (UserSettings()):IsUserFeatureEnabled("UserNavigationClickToMoveSkipPassedWaypoints")
		end
	)
	local FFlagUserNavigationClickToMoveSkipPassedWaypoints = FFlagUserNavigationClickToMoveSkipPassedWaypointsSuccess
		and FFlagUserNavigationClickToMoveSkipPassedWaypointsResult
	local Player = Players.LocalPlayer
	...
	local function Pather(endPoint, surfaceNormal, overrideUseDirectPath)
			...
			if seat and seat:IsA("VehicleSeat") then
				...
				if vehicle then
					...
					if directPathForVehicle then
						...
						agentRadius = AgentSizeIncreaseFactor
							* 0.5
				...
				if this.AgentCanFollowPath then
					if this.DirectPath then
						this.pointList = {
							...
							PathWaypoint.new(
								this.TargetPoint,
								this.DirectPathRiseFirst and Enum.PathWaypointAction.Jump
		...
		function this:IsCurrentWaypointReached()
			...
			if this.CurrentWaypointPlaneNormal ~= ZERO_VECTOR3 then
				local dist = this.CurrentWaypointPlaneNormal:Dot(this.CurrentHumanoidPosition)
					- this.CurrentWaypointPlaneDistance
		...
		function this:MoveToNextWayPoint(currentWaypoint, nextWaypoint, nextWaypointIdx)
			...
			this.CurrentWaypointPlaneNormal =
				Vector3.new(this.CurrentWaypointPlaneNormal.X, 0, this.CurrentWaypointPlaneNormal.Z)
		...
		function this:Start(overrideShowPath)
			...
			if #this.pointList > 0 then
				...
				this.CurrentHumanoidPosition = this.Humanoid.RootPart.Position + this.HumanoidOffsetFromPath
				this.CurrentHumanoidVelocity = this.Humanoid.RootPart.Velocity
				this.SeatedConn = this.Humanoid.Seated:Connect(function(isSeated, seat)
	...
	function ClickToMove:UpdateMovement(inputState)
		...
		elseif self.wasdEnabled then
			self.keyboardMoveVector =
				Vector3.new(self.leftValue + self.rightValue, 0, self.forwardValue + self.backwardValue)
		end
	end
	function ClickToMove:UpdateJump() end
	...
	function ControlModule:OnRenderStepped(dt)
		if self.activeController and self.activeController.enabled and self.humanoid then
			...
			if self.vehicleController then
				moveVector, vehicleConsumedInput =
					self.vehicleController:Update(moveVector, cameraRelative, self.activeControlModule == Gamepad)
			end
			if cameraRelative then
