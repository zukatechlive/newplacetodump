...
local function buildEnv(fakeGame, fakeWs)
	...
	env.load = function()
		error("[SHIELD] load() is blocked inside the sandbox")
	end
...
_SHIELD = {
	exec = function(source, chunkName)
		...
		if not fn then
			warn("[SHIELD] Compile error:", err)
			return false, err
		end
		local ok, runErr = pcall(fn)
		if not ok then
			warn("[SHIELD] Runtime error:", runErr)
			return false, runErr
		end
		return true
	end,
	execSandboxed = function(source, chunkName)
		...
		local ok, runErr = pcall(fn)
		if not ok then
			warn("[SHIELD] Sandbox runtime error:", runErr)
			return false, runErr
		end
		return true
	end,
	...
	inspect = function(fn)
		if type(fn) ~= "function" then
			warn("[SHIELD] inspect() expects a function, got " .. type(fn))
			return
		end
...
print("║  _SHIELD.exec(src)           run trusted     ║")
print("║  _SHIELD.execSandboxed(src)  run in box      ║")
print("║  _SHIELD.scan(src)           scan source     ║")
print("║  _SHIELD.bypass()            one-shot raw    ║")
print("║  _SHIELD.trust(name)         perm whitelist  ║")
print("║  _SHIELD.inspect(fn)         dump upvalues   ║")
print("║  _SHIELD.log([n])            dump audit log  ║")
print("║  _SHIELD.status()            show stats      ║")
print("║  _SHIELD.disable()           remove hook     ║")
