if getgenv()._AdonisLoaded then
	return
end
getgenv()._AdonisLoaded = true
...
local is_our = isourclosure or function()
	return false
end
...
local function getCachedGC()
	local now = os.clock()
	if gcCache and (now - gcCacheTime) < GC_TTL then
		return gcCache
	end
	...
	local function insertInCache(func, wrapped)
		...
		New = {
			...
			ReplacementFunc = function(...)
				...
				if not args[1] then
					local err = args[2]
					if err ~= secondError and New.WrapCount > stackThresholdMax then
						task.spawn(New.Gc)
						return getrenv().error(firstError, 2)
					elseif err == secondError or select(2, pcall(WrapHook(wrapped))) == secondError then
						task.spawn(New.Gc)
...
do
	...
	for _, fn in ipairs({ debug.getupvalues, debug.getlocals, debug.getconstants }) do
		if type(fn) == "function" then
			pcall(
				hook_fn,
				fn,
...
do
	...
	if s ~= "[C]" or l ~= -1 or n ~= "" or a ~= 0 then
		warn(
			string.format(
				"[Zuka] WARNING: newcclosure may not pass validity! source=%s line=%s name=%s args=%s",
				tostring(s),
				tostring(l),
				tostring(n),
				tostring(a)
...
local function sanitizeModule(moduleData)
	if type(moduleData) ~= "table" then
		return moduleData
	end
	for _, key in ipairs({ "Security", "Verify", "Check", "AntiCheat", "ExploitCheck", "Scan" }) do
	...
	for _, key in ipairs({ "Hash", "CheckSum", "Signature" }) do
		rawset(moduleData, key, nil)
	end
	return moduleData
end
	...
	new_cc(function(module)
		...
		if typeof(module) == "Instance" and module:IsA("ModuleScript") then
			...
			if module.Name == "1" and module.Parent and module.Parent.Name == "Settings" then
				local ok, data = pcall(_require, module)
				return ok and sanitizeModule(data) or data
			end
			...
			local uiPatterns = { "topbar", "icon", "adonis", "aethetic", "adminui", "cmdbar" }
			for _, pat in ipairs(uiPatterns) do
				if name:find(pat, 1, true) then
					return setmetatable({}, {
						__index = function()
							return function() end
...
do
	...
	oldBind = hook_fn(
		...
		new_cc(function(self, name, priority, callback)
			if not check_caller() then
				...
				local blocked = { "ac", "security", "verify", "anticheat", "check", "scan", "integrity" }
	...
	print("[Zuka] BindToRenderStep filter: active")
end

local AC_SIGNATURES = {
	{ "Detected", true, 2 },
	{ "RemovePlayer", true, 2 },
	{ "CheckAllClients", true, 1.5 },
	{ "AntiCheatEnabled", false, 1.5 },
	{ "KickedPlayers", false, 1 },
	{ "SpoofCheckCache", false, 1 },
	{ "ClientTimeoutLimit", false, 1 },
	{ "CharacterCheck", true, 1 },
	{ "UserSpoofCheck", true, 1 },
	{ "GetPlayer", true, 0.5 },
	{ "PlayerData", false, 0.5 },
	{ "BanList", false, 0.5 },
	{ "CommandLog", false, 0.5 },
	{ "ExploitLog", false, 1 },
	{ "FlagPlayer", true, 1 },
	{ "LogExploit", true, 1 },
	{ "WarnPlayer", true, 0.5 },
...
local function scoreTable(v)
	...
	if
		rawget(v, "Detected") == nil
		and rawget(v, "RemovePlayer") == nil
		and rawget(v, "SilentKick") == nil
		and rawget(v, "FlagPlayer") == nil
	then
		return 0
	end
...
local function hookACTable(tbl)
	...
	if type(tbl.Detected) == "function" then
		safeHook(tbl.Detected, function(player, action, info)
			...
			warn(string.format("[Zuka] Detection caught: %s | %s", tostring(action), tostring(info)))
		end)
	end
	...
	if type(tbl.RemovePlayer) == "function" then
		safeHook(tbl.RemovePlayer, function(p, info)
			...
			warn(string.format("[Zuka] RemovePlayer blocked: %s", tostring(info)))
		end)
	end

	if type(tbl.SilentKick) == "function" then
		safeHook(tbl.SilentKick, function(p, reason)
			Stats.KickAttempts += 1
			warn(string.format("[Zuka] SilentKick blocked: %s", tostring(reason)))
		end)
	end

	if type(tbl.FlagPlayer) == "function" then
		safeHook(tbl.FlagPlayer, function(p, reason)
			Stats.DetectionsCaught += 1
			warn(string.format("[Zuka] FlagPlayer blocked: %s", tostring(reason)))
		end)
	end

	if type(tbl.LogExploit) == "function" then
		safeHook(tbl.LogExploit, function(...) end)
	end
	...
	if type(tbl.UserSpoofCheck) == "function" then
		safeHook(tbl.UserSpoofCheck, function()
			return nil
		end)
	end
	...
	if type(tbl.WarnPlayer) == "function" then
		safeHook(tbl.WarnPlayer, function() end)
	end
	...
	if type(tbl.ExploitLog) == "table" then
		local mt = {}
		rawset(mt, "__index", function()
			return nil
		end)
		...
		pcall(setmetatable, tbl.ExploitLog, mt)
	end
end

local function patchRemoteClients()
...
local function installNamecallHook()
	...
	mt.__namecall = new_cc(function(self, ...)
		...
		if method == "Kick" and self == LocalPlayer then
			...
			for _, kw in ipairs(kickKeywords) do
				if msg:find(kw) then
					Stats.KickAttempts += 1
					warn(string.format("[Zuka] Kick blocked: %s", msg))
					return nil
				end
			end
		end
		...
		if method == "KickPlayer" and self == Players then
			...
			if target == LocalPlayer then
				Stats.KickAttempts += 1
				warn("[Zuka] Players:KickPlayer blocked")
				return nil
			end
		end
		...
		if method == "FireServer" or method == "InvokeServer" then
			...
			if shouldBlockRemote(name) then
				...
				warn(string.format("[Zuka] Remote blocked: %s:%s", name, method))
	...
	print("[Zuka] Namecall hook: active")
end
...
local function protectKick()
	...
	safeHook(origKick, function(self, reason, ...)
		...
		if self == LocalPlayer then
			...
			for _, kw in ipairs(kickKeywords) do
				if msg:find(kw) then
					Stats.KickAttempts += 1
					warn(string.format("[Zuka] Instance Kick blocked: %s", msg))
					return nil
				end
			end
		end
	...
	print("[Zuka] Kick protection: active")
end
...
local function rescan()
	...
	if tbl and tbl ~= cachedACTable then
		cachedACTable = tbl
		hookACTable(tbl)
		warn("[Zuka] New AC table found during rescan — hooked")
	end
	patchRemoteClients()
end
...
local function initialize()
	...
	if cachedACTable then
		...
		print("[Zuka] AC table found and hooked on init")
	else
		warn("[Zuka] AC table not found on init — will rescan")
	end
	...
	task.spawn(function()
		while not isUnloaded do
			...
			pcall(function()
				warn(
					string.format(
						"[Zuka] Stats | Kicks: %d | RemotesBlocked: %d | Detections: %d | ClientChecks: %d | Hooks: %d",
						Stats.KickAttempts,
	...
	print("[Zuka] Adonis bypass v4.0 initialized")
end

getgenv()._AdonisBypass = {
	Version = "4.0",
	...
	PrintStats = function()
		for k, v in pairs(getgenv()._AdonisBypass.GetStats()) do
	...
	BlockRemote = function(name)
		...
		warn(string.format("[Zuka] Manually blocked remote: %s", name))
	end,

	UnblockRemote = function(name)
		REMOTE_BLOCK_EXACT[name] = nil
		warn(string.format("[Zuka] Unblocked remote: %s", name))
	end,

	Unload = function()
		isUnloaded = true
		getgenv()._AdonisLoaded = nil
		...
		if originalFunctions.kick then
			pcall(hook_fn, LocalPlayer.Kick, originalFunctions.kick)
		end

		warn("[Zuka] Adonis bypass unloaded — originals restored")
	end,
}
