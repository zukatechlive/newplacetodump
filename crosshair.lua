...
CrosshairImg.Size = UDim2.new(0.4, 0, 0.4, 0)
...
local function hideDefaultMouseIcon()
...
local function updateCrosshairVisibility()
	local shiftLockActive = isShiftLockEnabled()

	if shiftLockActive then
		CrosshairImg.Visible = true
		hideDefaultMouseIcon()
	else
		CrosshairImg.Visible = false
	end
end
...
UserInputService:GetPropertyChangedSignal("MouseBehavior"):Connect(updateCrosshairVisibility)

RunService.RenderStepped:Connect(updateCrosshairVisibility)
